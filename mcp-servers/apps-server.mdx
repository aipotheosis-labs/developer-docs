...
```json Cursor
{
  "mcpServers": {
    "aipolabs-mcp-apps": {
        "command": "uvx",
        "args": ["aipolabs-mcp", "apps-server", "--apps", "<APP_1>,<APP_2>,...", "--linked-account-owner-id", "<LINKED_ACCOUNT_OWNER_ID>"],
        "env": {
...
```bash Run Locally
# Set API key
export AIPOLABS_ACI_API_KEY=<YOUR_AIPOLABS_ACI_API_KEY>

# Option 1: Run in stdio mode (default)
uvx aipolabs-mcp apps-server  --apps "<APP_1>,<APP_2>,..." --linked-account-owner-id <LINKED_ACCOUNT_OWNER_ID>

# Option 2: Run in sse mode
uvx aipolabs-mcp apps-server  --apps "<APP_1>,<APP_2>,..." --linked-account-owner-id <LINKED_ACCOUNT_OWNER_ID> --transport sse --port 8000
```
</CodeGroup>
