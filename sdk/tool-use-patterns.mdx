...
- **Pattern 2: Dynamic Tool Discovery and Execution**
    - **2.1: Tool List Expansion**
        - Provide all but the `ACIExecuteFunction` meta function (tool) to the LLM.
    - **2.2: Tool Definition as Text Context Approach**
        - Provide all meta functions (tools) to the LLM.
...
In this use case, the tools list provided to the LLM is static, all meta functions (tools) from the ACI.dev are included.
