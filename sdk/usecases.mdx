...
This is the most straightforward use case. You can directly find the functions you want to use on the developer portal, retrieve the function definitions, and append them to your LLM API call. This way your agents will only use the tools you have selected and provided, it would not attempt to find and use other tools.
```python
...
Unlike the semi autonomous case - where retrieved function definitions are appended — the Fully Autonomous mode embeds these definitions directly in the context window, requiring the LLM to decide on utilizing the ACIExecuteFunction meta function to execute an API call.

By using the meta functions in this way, the developer does not have to manage the tools list, however the accuracy of tool use can decrease.
